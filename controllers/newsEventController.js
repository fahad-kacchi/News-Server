...
const CustDetail = require('../models/newsEvents');
const FType=require('../models/newsEvents')
...
custDetail.create = async function (req, res, next) {
    try {
        ...
        const {title,subtitle,description,newsType,imagUrl,location,editor,jornalist,approveDate,newStatus} = req.body;
        var query = {title,subtitle,description,newsType,imagUrl,location,editor,jornalist,approveDate,newStatus};
        ...
        if (!resp) {
            ...
            res.json({
                status: true,
                message: 'News Event Inserted Successfully',
                result: newFile
            })
        }
         else {
