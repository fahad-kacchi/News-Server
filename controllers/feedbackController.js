...
const CustDetail = require('../models/feedback');
const FType=require('../models/feedback')
...
custDetail.create = async function (req, res, next) {
    try {
        ...
        const  {userEmail,message,newsID} = req.body;
        var query = {userEmail,message,newsID};
        ...
        if (!resp) {
            ...
            res.json({
                status: false,
                message: 'feedback is already registered',
                result: {}
            })
        }
    } catch (error) {
      ...
      .exec(function(err, List) {
        ...
        if (Array.from(List).length) {
          ...
          res.json(List);
            ...
            console.log('feedback are empty')
          res.json({
            message: 'feedback are empty'
          });
        }
      });
  };
...
custDetail.update = async function (req, res, next) {
    try {
      ...
      if (!req.body) {
        return res.status(400).send({
          message: "feedback Details to be updated can not be empty",
        });
      }
      ...
      if (!resp) {
        console.log("Unable to update feedback");
        res.json({
          status: false,
  ...
  custDetail.showById = async function (req, res, next) {
    try {
    const {
      _id
    } = req.body;
  ...
  custDetail.showNews = async function (req, res, next) {
    try {
    const {
      newsType
    } = req.body;
